...
def generate_acc(model, all_data, labels):

    print("About to predict...")
    Y_pred = model.predict(all_data, use_multiprocessing=True)
...
def main():
    ...
    # desired_len = len(data[0][0]) + len(data[0][1]) + len(data[0][2]) + len(data[0][3])

    # all_data = np.empty(shape=(desired_len, 128, 1250, 3))
    # labels = np.empty(shape=(desired_len), dtype=int)

    # counter = 0

    # for label in data[0]:
    #     for sample in label:
    #         all_data[counter] = np.repeat(sample[0][..., np.newaxis], 3, -1)
    #         labels[counter] = np.repeat(sample[0][..., np.newaxis], 3, -1)
    #         counter += 1
    ...
    ### eval
    generate_acc(model, all_data, labels)
