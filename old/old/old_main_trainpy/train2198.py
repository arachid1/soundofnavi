...
class timestep_metric_callback(tf.keras.callbacks.LambdaCallback):
...
def generate_timed_spec(filename, label, shape, initial_channels):
    ...
    spectrogram = tf.transpose(spectrogram.to_tensor())
    spectrogram = tf.math.pow(spectrogram, 1/3)
    # tf.print(tf.shape(spectrogram))
    # print(tf.shape(spectrogram))
    spectrogram = tf.reshape(spectrogram, (shape[0], shape[1], tf.cast(shape[2], dtype=tf.int32)))
    # tf.print(tf.shape(spectrogram))
    # print(tf.shape(spectrogram))
...
def train_model(train_file, job_dir, params, wav_params, spec_params):
    ...
    if concat:
        ...
        if augmentation:
            train_samples, train_labels = manage_pneumonia_augmentations(train_samples, wav_add, spec_add, wav_quantity, spec_quantity, job_version, wav_path, time_masking, frequency_masking, loudness, dataset_path, subfolder="augmented_3")
        ...
        val_dataset, train_dataset = process_data(val_samples, train_samples, generate_timed_spec, shape, batch_size, initial_channels)
    ...
    # callbacks
    metrics_callback = timestep_metric_callback(val_dataset, shape, n_classes, job_dir, min_delta, es_patience, patience, min_lr, factor, epoch_start, target, split_count, clause)
