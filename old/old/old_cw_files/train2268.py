...
def train_model(train_file, job_dir, params, wav_params, spec_params, model_to_be_trained, job_count):
    ...
    if concat:
        ...
        train_slice_one, train_slice_two = mixup_samples[:(len(mixup_samples)/2)], mixup_samples[(len(mixup_samples)/2):]

        train_slice = tf.data.Dataset.zip((train_slice_one, train_slice_two))

        mixup_batch_size = 1

        mixup_train_dataset = train_slice.map(lambda ds_one, ds_two: mix_up(ds_one, ds_two, batch_size=mixup_batch_size, alpha=0.2), num_parallel_calls=tf.data.AUTOTUNE)
