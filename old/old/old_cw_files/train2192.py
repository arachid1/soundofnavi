...
def conv2d(N_CLASSES, SR, BATCH_SIZE, LR, SHAPE, WEIGHT_DECAY, LL2_REG, EPSILON, LABEL_SMOOTHING, CB):
    ...
    opt = tf.keras.optimizers.Adam(
        learning_rate=CB, beta_1=0.9, beta_2=0.999, epsilon=EPSILON, decay=WEIGHT_DECAY, amsgrad=False
    )
...
def train_model(train_file, job_dir, params, wav_params, spec_params):
    ...
    if concat:
        ...
        # train_samples, val_samples = process_grouped_samples(grouped_train_samples, grouped_val_samples)
        # train_samples = []
        # val_samples = []
        # for s in grouped_train_samples:
        #     for recording in s[0]:
        #         train_samples.append([recording, s[1]])
        # for s in grouped_val_samples:
        #     for recording in s[0]:
        #         val_samples.append([recording, s[1]])
        print("-----------------------")
        ...
        # print("Pneumonia train recordings: {}, Non-pneumonia train recordings: {}".format(nb_train_pneumonia, nb_train_non_pneumonia))
        # print("There are {} pneumonia recordings and {} non-pneumonia recordings in training".format(nb_train_pneumonia, nb_train_non_pneumonia))

        if augmentation:
            train_samples = manage_pneumonia_augmentations(train_samples, wav_add, spec_add, wav_quantity, spec_quantity, job_version, wav_path, time_masking, frequency_masking, loudness, dataset_path)
        ...
        #     augmented_specs = apply_spec_augmentations(augmented_specs, os.path.join(dataset_path, "augmented"), len(augmented_specs), time_masking, frequency_masking, loudness, job_version, wav_path)
        ...
        # print("Number of Pneumonia val recordings: {}, Non-pneumonia val recordings: {}".format(nb_val_pneumonia, nb_val_non_pneumonia))
        print("-----------------------")

        lr = 1e-3 * (original_length / len(train_samples))

        print("The initial learning rate for this job is: {}".format(lr))
        # print("There are {} pneumonia recordings and {} non-pneumonia recordings in validation".format(nb_val_pneumonia, nb_val_non_pneumonia))
        ...
        val_dataset, train_dataset = process_data(val_samples, train_samples, generate_cuberooted_spec, shape, batch_size, initial_channels)
    ...
    # callbacks
    cb = tensorflow_addons.optimizers.CyclicalLearningRate(
        initial_learning_rate=min_lr,
        maximal_learning_rate=lr,
        step_size=2600, # nb_iterations = 263 -> [526, 2630] according to medium article/Leslie smith
        scale_fn=lambda x: 1 / (2.0 ** (x - 1)),
        scale_mode='cycle')
    metrics_callback = metric_callback(val_dataset, shape, n_classes, job_dir, min_delta, es_patience, patience, min_lr, factor, epoch_start, target, split_count, clause, adaptive_lr=False)
