...
def find_pneumonia_diagnosis(recording_annotations):
    ...
    if recording_annotations['Diagnosis'] == 'Pneumonia' or recording_annotations['Diagnosis'] == 'pneumonia':
        return 1
    else:
        return 0
...
def find_all_diagnosis(recording_annotations):
    if recording_annotations['Diagnosis'] == 'COPD':
        ...
        return 2
    elif recording_annotations['Diagnosis'] == 'Pneumonia' or (recording_annotations['Diagnosis'] == 'pneumonia'):
        return 1
    elif recording_annotations['Diagnosis'] == 'Healthy':
        return 0
...
def Extract_New_Pneumonia_Data(root):
    # recording_annotations = pd.read_csv(os.path.join(
    #     root, "Data_annotation.xlsx"))
    excel_path = os.path.join(root, "Data_annotation.xlsx")
...
def extract_new_wav(root, sr, overlap_threshold, length_threshold, audio_length, step_size, dataset=None, extension='.wav'):
    
    label_root = root
    root = os.path.join(root, 'Audio_Files')
    if dataset is None:
        print("Dataset is not specified. Exit.")
        exit()
    ...
    rec_annotations = Extract_New_Pneumonia_Data(label_root)
    ...
    for file_name in filenames:
        # print(file_name)
        patient_id = int(file_name.split('_')[0][2:]) - 1
        # print(patient_id)
        # int(file_name[:3])
        # if not ((rec_annotations.loc[patient_id]['Diagnosis'] == 'Healthy') or (rec_annotations.loc[patient_id]['Diagnosis'] == 'Pneumonia')):
        #     continue
...
def generate_new(icbhi_root, sr, overlap_threshold, length_threshold, audio_length, step_size, spec_type, n_fft, hop_length, spec_win_length, n_mels, height, width, coch_path, coch_params, wav_params, spec_params, augmentation):
    print("Generating ICBHI...")
    icbhi_data = extract_new_wav(icbhi_root, sr, overlap_threshold, length_threshold, audio_length, step_size, dataset="ICBHI")
    if augmentation:
...
def generate(params, wav_params, spec_params):
    ...
    icbhi_data = generate_new(**new_params_list)
    ...
    write_pneumonia_icbhi_to_txt_file(icbhi_data, txt_dataset_dest, "jordan")
