...
def generate_half_spec(filename, label, shape, initial_channels, cuberooting, normalizing):
    ...
    # tf.print("here")
    # tf.print(spectrogram[0])
    spectrogram, upper = tf.split(spectrogram, [64, 64])
    # tf.print(upper[0])
    # tf.print(lower[0])
    # tf.print(tf.shape(upper))
    # tf.print(tf.shape(lower))
    # tf.print(tf.reduce_sum(upper))
    # tf.print(tf.reduce_sum(lower))
...
def train_model(train_file, job_dir, params, wav_params, spec_params, model_to_be_trained, job_count):
    ...
    if concat:
        ...
        if augmentation:
            ...
            train_samples, train_labels = manage_label_augmentations(train_samples, wav_add, spec_add, 50, 100, job_version, wav_path, time_masking, frequency_masking, loudness, dataset_path, label=0)
