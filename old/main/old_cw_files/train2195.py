...
def train_model(train_file, job_dir, params, wav_params, spec_params):
    ...
    if concat:
        ...
        if augmentation:
            train_samples, train_labels = manage_pneumonia_augmentations(train_samples, wav_add, spec_add, wav_quantity, spec_quantity, job_version, wav_path, time_masking, frequency_masking, loudness, dataset_path)
    ...
    # callbacks
    step_size = int(len(train_samples)/batch_size)*6
    print("The step size for cyclical LR is {}".format(step_size))
    cb = tensorflow_addons.optimizers.CyclicalLearningRate(
        initial_learning_rate=min_lr,
        maximal_learning_rate=lr,
        step_size=step_size, # nb_iterations = 263 -> [526, 2630] according to medium article/Leslie smith
