...
def conv2d(N_CLASSES, SR, BATCH_SIZE, LR, SHAPE, WEIGHT_DECAY, LL2_REG, EPSILON, LABEL_SMOOTHING):
    ...
    x = Conv2DMoE(8, 2, KERNEL_SIZE, expert_activation='relu', padding='same', gating_activation='sigmoid')(x)
    x = layers.AveragePooling2D(pool_size=POOL_SIZE, padding="same")(x)
    x = layers.BatchNormalization()(x)
    x = Conv2DMoE(16, 2, KERNEL_SIZE, expert_activation='relu', padding='same', gating_activation='sigmoid')(x)
    x = layers.AveragePooling2D(pool_size=POOL_SIZE, padding="same")(x)
    x = layers.BatchNormalization()(x)
    x = Conv2DMoE(32, 2, KERNEL_SIZE, expert_activation='relu', padding='same', gating_activation='sigmoid')(x)
    x = layers.AveragePooling2D(pool_size=POOL_SIZE, padding="same")(x)
    x = layers.BatchNormalization()(x)
    x = Conv2DMoE(64, 2, KERNEL_SIZE, expert_activation='relu', padding='same', gating_activation='sigmoid')(x)
    x = layers.AveragePooling2D(pool_size=POOL_SIZE, padding="same")(x)
    x = layers.BatchNormalization()(x)
    x = Conv2DMoE(128, 2, KERNEL_SIZE, expert_activation='relu', padding='same', gating_activation='sigmoid')(x)
    x = layers.AveragePooling2D(pool_size=POOL_SIZE, padding="same")(x)
    x = layers.BatchNormalization()(x)
    x = Conv2DMoE(256, 2, KERNEL_SIZE, expert_activation='relu', padding='same', gating_activation='sigmoid')(x)
    x = layers.AveragePooling2D(pool_size=POOL_SIZE, padding="same")(x)
    x = layers.BatchNormalization()(x)
