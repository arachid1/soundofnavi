...
def convert_weight_labels(label):
    if label == [1,0]:
        return 0 
    elif label == [0,1]:
        return 1
...
def train_model(train_file, **args):
    ...
    with tf.device('/CPU:0'): 
        ...
        for folder_path in glob.glob(os.path.join(root, "*"), recursive=True):
            ...
            labels.append(one_hot_encode(convert_pneumonia_label(label)))
        ...
        #         print(_file)
        #         filedata = open(_file, encoding='UTF-8').read() 
        #         # except:
        #         #     print("error")
        #         # print(_file)
        #         # x = tf.io.read_file(_file)
        # exit()
        ...
        if bool(params["CLASS_WEIGHTS"]):
            ...
            y_train = []
            for label in labels:
                y_train.append(convert_weight_labels(label))
