...
def generate_spec_wcuberooting(filename, label, shape, initial_channels):
...
def train_model(train_file, job_dir, params, wav_params, spec_params):
    ...
    if concat:
        ...
        if spec_add:
            ...
            augmented_specs = apply_augmentations(augmented_specs, os.path.join(dataset_path, "augmented_2"), len(augmented_specs), time_masking, frequency_masking, loudness, job_version, wav_path)
        ...
        val_dataset, train_dataset = process_data(val_samples, train_samples, generate_spec_wcuberooting, shape, batch_size, initial_channels)
