...
def train_model(train_file, **args):
    ...
    # [wav_path : list, wav_quantity, [spec_add, spec_quantity, time_masking, frequency_masking, loudness]]
    # -> wav_path, or the dataset to use -> v36, v37, v38, [v36, v37]
    # -> quantity, or how much to add -> 500, 1000, 2000
    # -> parameters for spec addition or not
    # NB: adjust lr to quantity

    job_list = [[['v36','v37'], 500, [0, 0, 0, 0, 0]], [['v36'], 1000, [0, 0, 0, 0, 0]], [['v37'], 1000, [0, 0, 0, 0, 0]], [['v37'], 2000, [0, 0, 0, 0, 0]], ]
    # ['v38'], 500, [0, 0, 0, 0, 0]]]
    
    for job_params in job_list:

        wav_path = job_params[0]
        wav_quantity = job_params[1]
        spec_add = job_params[-1][0]
        spec_quantity = job_params[-1][1]
        time_masking = job_params[-1][2]
        frequency_masking = job_params[-1][3]
        loudness = job_params[-1][4]

        print("The parameters for job N: {} were: {}".format(split_count, job_params))

        run = wandb.init(project="augmentation_model9", mode="disabled", sync_tensorboard=False, reinit=True)
        ...
        if concat:
            ...
            if wav_add:
                ...
                for wav_folder in wav_path:
                    ...
                    for i in range(len(augmented_audios)):
                        augmented_audios[i][0] = augmented_audios[i][0].replace(params["PARAM"], wav_folder) # TODO: rename some variables
            ...
            print("The learning rate for this job is: {}".format(lr))
        ...
        model_params['CB'] = cyclical_lr_callback
        # model setting
