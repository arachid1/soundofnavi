...
def train_model(train_file, job_dir, params, wav_params, spec_params, model_to_be_trained):
    ...
    if concat:
        ...
        if augmentation:
            subfolder_1 = os.path.join(dataset_path, 'augmented')
            subfolder_2 = os.path.join(dataset_path, 'augmented_2')
            if os.path.exists(subfolder_1):
                shutil.rmtree(subfolder_1)
            if os.path.exists(subfolder_2):
                shutil.rmtree(subfolder_2)
            train_samples, train_labels = manage_label_augmentations(train_samples, wav_add, spec_add, wav_quantity, spec_quantity, job_version, wav_path, time_masking, frequency_masking, loudness, dataset_path, label=1, subfolder='augmented')
            train_samples, train_labels = manage_label_augmentations(train_samples, wav_add, spec_add, 50, 100, job_version, wav_path, time_masking, frequency_masking, loudness, dataset_path, label=0, subfolder='augmented_2')
