...
class custom_metric_callback(tf.keras.callbacks.LambdaCallback):
    ...
    def __init__(self, validation_data, shape, n_classes, job_dir, min_delta, es_patience, lr_patience, min_lr, factor, epoch_start, target, split_count, clause, adaptive_lr=True, average_mode='binary', clause_portion=0.9):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for batch, y_true in self.validation_data:
            ...
            preds = preds.reshape((len(preds), self.n_classes))
        ...
        converted_y_true_all = np.array(y_true_all).argmax(axis=1)
        converted_y_pred_all = np.array(y_pred_all).argmax(axis=1)
        cm = confusion_matrix(converted_y_true_all, converted_y_pred_all)
        normalized_cm = confusion_matrix(converted_y_true_all, converted_y_pred_all, normalize='true')
        cm_sum = np.sum(cm)
        acc = np.trace(cm) / cm_sum
        class_accuracies=self.compute_class_accuracies(cm)

        precision, recall, f1, support = precision_recall_fscore_support(converted_y_true_all, converted_y_pred_all, average=self.average_mode)
...
def train_model(train_file, job_dir, params, wav_params, spec_params):
    ...
    for i in range(0, 8):
        print("Label {} has {} samples".format(i, labels.count(i)))
    ...
    if concat:
        ...
        val_dataset, train_dataset = process_data(val_samples, train_samples, generate_spec, shape, batch_size, initial_channels, cuberooting, normalizing=1)
    ...
    # callbacks
    metrics_callback = custom_metric_callback(val_dataset, shape, n_classes, job_dir, min_delta, es_patience, patience, min_lr, factor, epoch_start, target, split_count, clause, adaptive_lr=adaptive_lr, average_mode='micro')
...
if __name__ == "__main__":
    ...
    # 2
    wav_params['WEIGHT_DECAY'] = float(1e-6)
    print("-----------------------")
    print("Parameters: {}".format(params))
    print("-----------------------")
    train_model(train_file, job_dir, params, wav_params, spec_params)
    # 3
    wav_params['WEIGHT_DECAY'] = float(1e-3)
    wav_params['LABEL_SMOOTHING'] = float(0.4)
    print("-----------------------")
    print("Parameters: {}".format(params))
    print("-----------------------")
    train_model(train_file, job_dir, params, wav_params, spec_params)
