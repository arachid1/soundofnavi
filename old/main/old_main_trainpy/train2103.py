...
def conv2d(N_CLASSES, SR, BATCH_SIZE, LR, SHAPE, WEIGHT_DECAY, LL2_REG, EPSILON, LABEL_SMOOTHING):
    ...
    # x = DenseMoE(512, 6, expert_activation='relu', gating_activation='relu')(x)
...
def train_model(train_file, **args):
    ...
    for train_indexes, val_indexes in kf.split(samples):

        run = wandb.init(project="kfold_crossvalidation_mixednet_{}".format(split_count), mode="disabled", sync_tensorboard=False, reinit=True)
        # print("here")
        # print(val_indexes)

        # grouped_val_samples = samples[val_indexes]
        # grouped_train_samples = samples[train_indexes]
        grouped_val_samples = [samples[i] for i in val_indexes]
        grouped_train_samples = [samples[i] for i in train_indexes]
        # print(grouped_train_samples)
        # print(grouped_val_samples[0])
        # print(grouped_val_samples[1])
        # exit()
        ...
        # print("PUT BACK THE GPUUUUUUUUUUUUUUUUUU")

        model.save(job_dir + "/model_{}.h5".format(split_count))

        excel_dest = job_dir + "/validation_sheet_{}.xls".format(split_count)
        generate_bangladesh_sheet(model, grouped_val_samples, excel_dest, six, initial_channels)
        split_count += 1
        run.finish()
