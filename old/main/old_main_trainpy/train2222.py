...
def model3(N_CLASSES, SR, BATCH_SIZE, LR, SHAPE, INITIAL_CHANNELS, WEIGHT_DECAY, LL2_REG, EPSILON, LABEL_SMOOTHING):
    ...
    # x = layers.BatchNormalization()(x)
    # x = layers.Dropout(0.1)(x)
    tower_1 = layers.Conv2D(48, (1,1), padding='same', activation='relu')(x)
    tower_1 = layers.Conv2D(48, (3,3), padding='same', activation='relu')(tower_1)
    tower_2 = layers.Conv2D(48, (1,1), padding='same', activation='relu')(x)
    tower_2 = layers.Conv2D(48, (5,5), padding='same', activation='relu')(tower_2)
    tower_3 = layers.MaxPooling2D((3,3), strides=(1,1), padding='same')(x)
    tower_3 = layers.Conv2D(48, (1,1), padding='same', activation='relu')(tower_3)
    x = layers.Concatenate(axis=-1)([tower_1, tower_2, tower_3])
    x = layers.AveragePooling2D(pool_size=POOL_SIZE, padding="same")(x)
    x = layers.BatchNormalization()(x)
    x = layers.Dropout(0.1)(x)
...
def model2(N_CLASSES, SR, BATCH_SIZE, LR, SHAPE, INITIAL_CHANNELS, WEIGHT_DECAY, LL2_REG, EPSILON, LABEL_SMOOTHING):
    ...
    x = layers.BatchNormalization()(x)
    x = InvertedResidual(filters=64, strides=1, expansion_factor=3, kernel_size=3)(x)
    x = InvertedResidual(filters=64, strides=1, expansion_factor=3, kernel_size=3)(x)
    x = layers.AveragePooling2D(pool_size=POOL_SIZE, padding="same")(x)
    x = layers.BatchNormalization()(x)
    x = InvertedResidual(filters=128, strides=1, expansion_factor=3, kernel_size=3)(x)
    x = InvertedResidual(filters=128, strides=1, expansion_factor=3, kernel_size=3)(x)
    x = layers.AveragePooling2D(pool_size=POOL_SIZE, padding="same")(x)
    x = layers.BatchNormalization()(x)
    x = InvertedResidual(filters=256, strides=1, expansion_factor=3, kernel_size=3)(x)
    x = InvertedResidual(filters=256, strides=1, expansion_factor=3, kernel_size=3)(x)
    x = layers.AveragePooling2D(pool_size=POOL_SIZE, padding="same")(x)
    x = layers.BatchNormalization()(x)
    x = layers.TimeDistributed(layers.GlobalAveragePooling1D())(x)
...
def model1(N_CLASSES, SR, BATCH_SIZE, LR, SHAPE, INITIAL_CHANNELS, WEIGHT_DECAY, LL2_REG, EPSILON, LABEL_SMOOTHING):
...
def train_model(train_file, job_dir, params, wav_params, spec_params, model_to_train):
    ...
    # model setting
    model = model_to_train(**model_params)
...
if __name__ == "__main__":
    ...
    print("-----------------------")
    train_model(train_file, job_dir, params, wav_params, spec_params, model1)
    print("-----------------------")
    print("Second model")
    print("-----------------------")
    train_model(train_file, job_dir, params, wav_params, spec_params, model2)
    print("-----------------------")
    print("Third model")
    print("-----------------------")
    train_model(train_file, job_dir, params, wav_params, spec_params, model3)
