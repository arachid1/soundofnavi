...
from .modules.main.global_helpers import visualize_audio
...
def train_model(datasets, model_to_be_trained, spec_aug_params, audio_aug_params, parse_function):
    ...
    print(np.max(spec))
    print(np.min(spec))
    visualize_audio(spec, os.path.join(parameters.job_dir, "audio_c/"), title=el[2])
    spec = spec / np.max(spec)
    print(np.max(spec))
    print(np.min(spec))
    visualize_audio(spec, os.path.join(parameters.job_dir, "audio_c/"), title=el[2] + str("_normalized"))
    exit()
