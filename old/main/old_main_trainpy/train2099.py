...
def parse_function_with_norm(filename, label, shape):
...
def train_model(train_file, **args):
    ...
    with tf.device('/CPU:0'): 
        ...
        #     arr = np.loadtxt(sample[0], delimiter=',')
        #     avg_spectrogram += arr
        # visualize_spectrogram(avg_spectrogram / len(val_samples), sr, "val_2077")

        # avg_spectrogram = np.zeros((shape[0], shape[1]))
        # for sample in train_samples:
        #     arr = np.loadtxt(sample[0], delimiter=',')
        #     avg_spectrogram += arr
        # visualize_spectrogram(avg_spectrogram / len(train_samples), sr, "train_2077")
        ...
        train_dataset = train_dataset.map(lambda filename, label: parse_function_with_norm(filename, label, shape), num_parallel_calls=4)
        ...
        val_dataset = val_dataset.map(lambda filename, label: parse_function_with_norm(filename, label, shape), num_parallel_calls=4)
        # val_dataset = val_dataset.map(parse_function, num_parallel_calls=4)
        val_dataset = val_dataset.batch(batch_size*3)
