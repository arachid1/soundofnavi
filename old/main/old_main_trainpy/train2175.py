    ...
    with tf.device('/CPU:0'): 
        ...
        if concat:
            ...
            if bool(wav_params['ADD']):
                ...
                augmented_indices = random.sample(range(len(train_samples)), int(wav_params['QUANTITY']))
                augmented_audios = [train_samples[i] for i in augmented_indices]
                for i in range(len(augmented_audios)):
                    # print(augmented_audios[i])
                    augmented_audios[i][0] = augmented_audios[i][0].replace(params["PARAM"], wav_params["WAV_PATH"])
                    # print(augmented_audios[i])
                    # exit()
                train_samples += augmented_audios
                print("Length of training samples after augmentation: {}".format(len(train_samples)))
            
            lr = lr * ((len(train_samples) - int(wav_params['QUANTITY']))/len(train_samples))
            print("The new LR is: {}".format(lr))
