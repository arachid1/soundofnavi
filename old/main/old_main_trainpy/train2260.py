...
def lstm_model1(N_CLASSES, SR, BATCH_SIZE, LR, SHAPE, INITIAL_CHANNELS, WEIGHT_DECAY, LL2_REG, EPSILON, LABEL_SMOOTHING):
    ...
    tower_1 = layers.TimeDistributed(layers.Conv2D(32, (1,1), padding='same', activation='relu'))(x)
    tower_1 = layers.TimeDistributed(layers.Conv2D(32, (3,3), padding='same', activation='relu'))(tower_1)
    tower_2 = layers.TimeDistributed(layers.Conv2D(32, (1,1), padding='same', activation='relu'))(x)
    tower_2 = layers.TimeDistributed(layers.Conv2D(32, (5,5), padding='same', activation='relu'))(tower_2)
    tower_3 = layers.TimeDistributed(layers.MaxPooling2D((3,3), strides=(1,1), padding='same'))(x)
    tower_3 = layers.TimeDistributed(layers.Conv2D(32, (1,1), padding='same', activation='relu'))(tower_3)
    x = layers.Concatenate(axis=-1)([tower_1, tower_2, tower_3])
    x = layers.TimeDistributed(layers.AveragePooling2D(pool_size=POOL_SIZE, padding="same"))(x)
    x = layers.BatchNormalization()(x)
    # x = layers.Dropout(0.1)(x)
    # x = TimeDistributedInvertedResidual(filters=32, strides=1, kernel_size=KERNEL_SIZE)(x)
    # x = TimeDistributedInvertedResidual(filters=32, strides=1, kernel_size=KERNEL_SIZE)(x)
    # x = layers.TimeDistributed(layers.AveragePooling2D(pool_size=POOL_SIZE, padding="same"))(x)
    # x = layers.BatchNormalization()(x)
    # x = layers.TimeDistributed(layers.Conv2D(16, (3, 3), padding='same', activation='relu'))(i)
    # x = layers.TimeDistributed(MixDepthGroupConvolution2D(kernels=KERNELS))(x)
    # x = layers.TimeDistributed(layers.BatchNormalization())(x)
    # x = layers.TimeDistributed(layers.Conv2D(32, (3, 3), padding='same', activation='relu'))(i)
    # x = layers.TimeDistributed(MixDepthGroupConvolution2D(kernels=KERNELS))(x)
    # x = layers.TimeDistributed(layers.BatchNormalization())(x)
    # x = layers.TimeDistributed(layers.AveragePooling2D(pool_size=POOL_SIZE, padding="same"))(x)
...
def lstm_model2(N_CLASSES, SR, BATCH_SIZE, LR, SHAPE, INITIAL_CHANNELS, WEIGHT_DECAY, LL2_REG, EPSILON, LABEL_SMOOTHING):
    ...
    x = layers.TimeDistributed(layers.AveragePooling2D(pool_size=POOL_SIZE, padding="same"))(x)
    x = layers.BatchNormalization()(x)
    tower_1 = layers.TimeDistributed(layers.Conv2D(64, (1,1), padding='same', activation='relu'))(x)
    tower_1 = layers.TimeDistributed(layers.Conv2D(64, (3,3), padding='same', activation='relu'))(tower_1)
    tower_2 = layers.TimeDistributed(layers.Conv2D(64, (1,1), padding='same', activation='relu'))(x)
    tower_2 = layers.TimeDistributed(layers.Conv2D(64, (5,5), padding='same', activation='relu'))(tower_2)
    tower_3 = layers.TimeDistributed(layers.MaxPooling2D((3,3), strides=(1,1), padding='same'))(x)
    tower_3 = layers.TimeDistributed(layers.Conv2D(64, (1,1), padding='same', activation='relu'))(tower_3)
    x = layers.Concatenate(axis=-1)([tower_1, tower_2, tower_3])
    x = layers.TimeDistributed(layers.AveragePooling2D(pool_size=POOL_SIZE, padding="same"))(x)
    x = layers.BatchNormalization()(x)
    # x = layers.Dropout(0.1)(x)
...
def lstm_model3(N_CLASSES, SR, BATCH_SIZE, LR, SHAPE, INITIAL_CHANNELS, WEIGHT_DECAY, LL2_REG, EPSILON, LABEL_SMOOTHING):
...
def lstm_model4(N_CLASSES, SR, BATCH_SIZE, LR, SHAPE, INITIAL_CHANNELS, WEIGHT_DECAY, LL2_REG, EPSILON, LABEL_SMOOTHING):
    ...
    i = layers.Input(shape=SHAPE + (INITIAL_CHANNELS,))
    x = layers.BatchNormalization()(i)
    x = layers.TimeDistributed(layers.Conv2D(8, (3, 3), padding='same', activation='relu'))(x)
    x = layers.TimeDistributed(MixDepthGroupConvolution2D(kernels=KERNELS))(x)
    x = layers.TimeDistributed(layers.BatchNormalization())(x)
    tower_1 = layers.TimeDistributed(layers.Conv2D(16, (1,1), padding='same', activation='relu'))(x)
    tower_1 = layers.TimeDistributed(layers.Conv2D(16, (3,3), padding='same', activation='relu'))(tower_1)
...
def lstm_model5(N_CLASSES, SR, BATCH_SIZE, LR, SHAPE, INITIAL_CHANNELS, WEIGHT_DECAY, LL2_REG, EPSILON, LABEL_SMOOTHING):
...
def lstm_model6(N_CLASSES, SR, BATCH_SIZE, LR, SHAPE, INITIAL_CHANNELS, WEIGHT_DECAY, LL2_REG, EPSILON, LABEL_SMOOTHING):
...
def train_model(train_file, job_dir, params, wav_params, spec_params, model_to_be_trained, job_count):
    ...
    shape = (portions_nb, int(shape[1]/portions_nb), shape[0])
...
if __name__ == "__main__":
    ...
    print("-----------------------")
    train_model(train_file, job_dir, params, wav_params, spec_params, lstm_model1, job_count=0)
    print("-----------------------")
    print("New")
    train_model(train_file, job_dir, params, wav_params, spec_params, lstm_model2, job_count=1)
    print("-----------------------")
    print("New")
    train_model(train_file, job_dir, params, wav_params, spec_params, lstm_model3, job_count=2)
    print("-----------------------")
    print("New")
    train_model(train_file, job_dir, params, wav_params, spec_params, lstm_model4, job_count=3)
    print("-----------------------")
    print("New")
    train_model(train_file, job_dir, params, wav_params, spec_params, lstm_model5, job_count=4)
    print("-----------------------")
    print("New")
    train_model(train_file, job_dir, params, wav_params, spec_params, lstm_model6, job_count=5)
    print("-----------------------")
