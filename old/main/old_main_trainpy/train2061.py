...
def parse_pneumonia_function_2(files, label, shape):
    ...
    for filename in files:
        ...
        spectrogram = tf.reshape(spectrogram.to_tensor(), (shape[1], shape[2]))
...
def train_model(train_file, **args):
    ...
    with tf.device('/CPU:0'): 
        ...
        train_dataset = train_dataset.map(lambda files, label: parse_pneumonia_function_2(files, label, shape), num_parallel_calls=4)
        ...
        val_dataset = val_dataset.map(lambda files, label: parse_pneumonia_function_2(files, label, shape), num_parallel_calls=4)
