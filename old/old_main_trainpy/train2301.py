...
def cutmix(train_ds_one, train_ds_two, shape):
    ...
    lambda_value = tf.random.uniform(shape=[], minval=0.3, maxval=0.7)
...
def apply_cutmix_augmentation(train_dataset, both_classes, repeat_factor, cutmix_quantity, shape):
    ...
    if both_classes:
        ln = len(list(train_dataset.as_numpy_iterator()))
        print("length before removing zero-pads: " + str(ln))
        train_dataset = train_dataset.filter(lambda x, y: tf.numpy_function(remove_zero_padded, [x, shape], tf.bool))
        ln = len(list(train_dataset.as_numpy_iterator()))
        print("length after removing zero-pads: " + str(ln))
...
def train_model(train_file, job_dir, params, wav_params, spec_params, model_to_be_trained, job_count, pneumonia_augmix_quantity, non_pneumonia_augmix_quantity, repeat_factor):
    ...
    if concat:
        ...
        for i in range(10):
            folder = 'cutmix_to_show/{}_no0pad_32boxstart'.format(i)
    ...
    final_length = len(list(train_dataset.as_numpy_iterator()))
    train_dataset = train_dataset.shuffle(final_length)
    train_dataset = train_dataset.batch(batch_size)
    train_dataset = train_dataset.prefetch(1)

    print("Final length of dataset: {}".format(final_length))
...
if __name__ == "__main__":
    ...
    print("-----------------------")
    print("Repeat factor of 4")
    train_model(train_file, job_dir, params, wav_params, spec_params, mixednet, 1, 158, 400, repeat_factor=2)
    print("-----------------------")
    print("Repeat factor of 4")
    train_model(train_file, job_dir, params, wav_params, spec_params, mixednet, 1, 158, 400, repeat_factor=3)
    print("-----------------------")
    print("Repeat factor of 4")
    train_model(train_file, job_dir, params, wav_params, spec_params, mixednet, 1, 158, 400, repeat_factor=4)
    # rint("-----------------------")
    # train_model(train_file, job_dir, params, wav_params, spec_params, mixednet, 0, 158, 400, 3)
    # print("-----------------------")
    # print("Repeat factor of 4")
    # train_model(train_file, job_dir, params, wav_params, spec_params, mixednet, 1, 4)
    # print("-----------------------")
    # print("Repeat factor of 5")
    # train_model(train_file, job_dir, params, wav_params, spec_params, mixednet, 2, 5)
    # print("-----------------------")
