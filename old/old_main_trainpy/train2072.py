...
def train_model(train_file, **args):
    ...
    with tf.device('/CPU:0'): 
        ...
        train_dataset = train_dataset.map(lambda filename, label: parse_function_for_val(filename, label, shape), num_parallel_calls=4)
        ...
        val_dataset = val_dataset.map(lambda filename, label: parse_function_for_val(filename, label, shape), num_parallel_calls=4)
    ...
    model.load_weights('../../cache/conv___04_0121___all_sw_coch_preprocessed_v2_param_v29_augm_v0_cleaned_8000___test___2049/third.h5')
