...
def train_model(train_file, **args):
    ...
    with tf.device('/CPU:0'): 
        ...
        if concat:
            ...
            pneumonia_samples = [(recording, s[1]) for s in grouped_train_samples if s[1] == 1 for recording in s[0]]
            non_pneumonia_samples = [(recording, s[1]) for s in grouped_train_samples if s[1] == 0 for recording in s[0]]
            random.shuffle(non_pneumonia_samples)
            non_pneumonia_samples = non_pneumonia_samples[:len(pneumonia_samples)]
            print(len(pneumonia_samples))
            print(len(non_pneumonia_samples))
            # exit()
            # oversampled_pneumonia_samples = []
            # for i in range(0, 3):
            #     for sample in pneumonia_samples:
            #         oversampled_pneumonia_samples.append(sample)
            # oversampled_pneumonia_samples = oversampled_pneumonia_samples[:len(non_pneumonia_samples)]
            train_samples = pneumonia_samples + non_pneumonia_samples
        ...
        metrics_callback = metric_callback(val_dataset, shape, n_classes, job_dir, min_delta, es_patience, patience, min_lr, factor, epoch_start, target)
    ...
    models = sorted(glob.glob(os.path.join(job_dir, "*.h5")))
    ...
    for model_path in models:
        ...
        bd_f1, bd_accuracy, bd_precision, bd_recall, bd_auc = generate_bangladesh_sheet(model, grouped_val_samples, excel_dest, six, initial_channels)
        if target == 0: # auc 
