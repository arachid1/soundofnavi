...
def conv2d(N_CLASSES, SR, BATCH_SIZE, LR, SHAPE, WEIGHT_DECAY, LL2_REG, EPSILON, NB_BLOCKS, KERNEL_SIZE, POOL_SIZE, DROPOUT, PADDING, DENSE_LAYER, CHANNELS):
    ...
    x = create_3_convs_relu_avg_bn(i, KERNEL_SIZE, POOL_SIZE, CHANNELS, PADDING)
    for y in range(NB_BLOCKS - 1):
        CHANNELS = CHANNELS * 2
        x = create_3_convs_relu_avg_bn(x, KERNEL_SIZE, POOL_SIZE, CHANNELS, PADDING)
    x = layers.Flatten()(x)
    if DENSE_LAYER != 0:
...
class data_generator(tf.keras.utils.Sequence):
    def __init__(
        self, wav_files, sr, n_classes, shape, batch_size, shuffle=True,
    ):
...
class cm_callback(tf.keras.callbacks.LambdaCallback):
    ...
    def on_train_end(self, logs=None):
        ...
        print("Highest accuracy: {}".format(self.highest_acc))
