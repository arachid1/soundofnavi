...
print("yoo: {}".format(tf.executing_eagerly()))
...
def train_model(train_file, **args):
    ...
    # print(augm_params)
    augm_ratio = float(augm_params["AUGM_RATIO"])
    time_masking = int(augm_params["TIME_MASKING"])
    frequency_masking = int(augm_params["FREQUENCY_MASKING"])
    loudness = int(augm_params["LOUDNESS"])
    # print(augm_ratio)
    # print(time_masking)
    ...
    # [augm_ratio, time_masking, frequency_masking, loudness]

    # augm_params_lst = [[0.2, 1, 1, 1], [0.4, 1, 1, 1], [0.2, 0, 1, 1], [0.4, 0, 1, 1], [0.8, 1, 1, 1], [0.6, 1, 1, 0]]
    # augm_params_lst = [[0.2, 1, 1, 1, 0.75, (0, 1)], [0.4, 1, 1, 1, 0.75, (0, 1)], [0.2, 0, 1, 1, 0.75, (0, 1)], [0.4, 0, 1, 1, 0.75, (0, 1)], [0.8, 1, 1, 1, 0.75, (0, 1)], [0.6, 1, 1, 0, 0.75, (0, 1)]]
    augm_params_lst = [[0.1, 0, 1, 0, 0.75, (0, 1)], [0.1, 0, 0, 1, 0.75, (0, 1)]]

    for augm_params_lst in augm_params_lst:

        run = wandb.init(project="kfold_augm_model9", mode="disabled", sync_tensorboard=False, reinit=True)

        augm_ratio = augm_params_lst[0]
        time_masking = augm_params_lst[1]
        frequency_masking = augm_params_lst[2]
        loudness = augm_params_lst[3]
        coverage_threshold = augm_params_lst[4]
        zone = augm_params_lst[5]
        print("The parameters for augmentation N: {} were: {}".format(split_count, augm_params_lst))
        ...
        if concat:
            ...
            augmented_length = math.floor(augm_ratio * len(train_samples))
            augmented_indices = random.sample(range(len(train_samples)), augmented_length)
            augmented_samples = [train_samples[i] for i in augmented_indices] 

            augmented_samples = apply_augmentations(augmented_samples, os.path.join(dataset_path, "augmented"), len(augmented_samples), time_masking, frequency_masking, loudness, coverage_threshold, zone)
            ...
            # augmented_dataset = process_augmented_data(augmented_samples, shape, initial_channels, batch_size, generate_spec, augmented_length, time_masking, frequency_masking, loudness)
        ...
        model.save(job_dir + "/{}/model_epoch{}.h5".format(split_count, n_epochs))
        ...
        for model_path in models:
            for pneumonia_threshold in thresholds:
                ...
                print("Epoch: {} & threshold: {}".format(epoch, pneumonia_threshold))
                excel_dest = job_dir + "/{}/validation_sheet_epoch{}.xls".format(split_count, epoch) #TODO: Change this in orig
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--job-dir",
        help="local/GCS location to save checkpoints, logs and others",
        required=True,
    )
    parser.add_argument(
        "--params",
