...
# # @tf.function
# def custom_mix_upp(ds_one, ds_two, batch_size, alpha=0.2):
...
#     images = images_two_mod + images_one_mod 
...
#   m = np.random.uniform(low=0.3, high=0.7)
# #   print("new")
# #   print(path)
...
def train_model(train_file, job_dir, params, wav_params, spec_params, model_to_be_trained, job_count, pneumonia_augmix_quantity, non_pneumonia_augmix_quantity, repeat_factor):
    ...
    if concat:
        ...
        train_samples_with_augmix = train_samples

        if augmentation: 
            pneumonia_augmix_samples = apply_augmix_augmentation(train_samples, pneumonia_augmix_quantity, dataset_path, job_version, wav_path, 1, shape, minval, maxval)
            non_pneumonia_augmix_samples = apply_augmix_augmentation(train_samples, non_pneumonia_augmix_quantity, dataset_path, job_version, wav_path, 0, shape, minval, maxval)
            train_samples_with_augmix.extend(pneumonia_augmix_samples)
            train_samples_with_augmix.extend(non_pneumonia_augmix_samples)

        print("Length of training samples after augmix augmentation: {}".format(len(train_samples_with_augmix)))
        ...
        f = generate_spec
        _____, temp_train_dataset = process_data(val_samples, train_samples, f, shape, batch_size, initial_channels, cuberooting, normalizing=normalizing)

        temp_train_dataset = temp_train_dataset.flat_map(lambda x, y: tf.data.Dataset.from_tensor_slices((x, y)))
        ...
        if augmentation:
            mixup_train_dataset = apply_mixup_augmentation(temp_train_dataset, both_classes, repeat_factor, mixup_quantity, shape, minval, maxval, no_zeropad=True)

        val_dataset, train_dataset = process_data(val_samples, train_samples_with_augmix, f, shape, batch_size, initial_channels, cuberooting, normalizing=normalizing)
