...
def new_model(N_CLASSES, SR, BATCH_SIZE, LR, SHAPE, INITIAL_CHANNELS, WEIGHT_DECAY, LL2_REG, EPSILON, LABEL_SMOOTHING):
    ...
    x = RandomFourierFeatures(
            output_dim=128, scale=10.0, kernel_initializer="gaussian"
        )(x)
    x = layers.Flatten()(x)
    ...
    model = tf.keras.Model(inputs=i, outputs=o, name="conv2d")
...
def generate_samples(audios_c_dict):
    samples = []
    for key, dict_samples in audios_c_dict.items():
        samples.extend(dict_samples)
    return samples

def create_tf_dataset(samples, parse_func, batch_size, shuffle):
...
if __name__ == "__main__":
    ...
    spec_aug_params = []
    audio_aug_params = []
    train_model(new_model, spec_aug_params, audio_aug_params)
