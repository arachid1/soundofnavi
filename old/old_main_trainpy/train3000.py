...
def conv2d(N_CLASSES, SR, BATCH_SIZE, LR, SHAPE, WEIGHT_DECAY, LL2_REG, EPSILON, KERNEL_SIZE, POOL_SIZE, DROPOUT, PADDING, DENSE_LAYER, CHANNELS):

    print("inside")
    KERNEL_SIZE = (int(KERNEL_SIZE), int(KERNEL_SIZE))
    POOL_SIZE = (int(POOL_SIZE), int(POOL_SIZE))
    DROPOUT = int(DROPOUT)
    # int(PADDING) == 0 ? PADDING = "same": PADDING = "valid"
    PADDING = "same" if int(PADDING) == 0 else "valid"
    inp = layers.Input(shape=SHAPE, batch_size=BATCH_SIZE)
    x = layers.BatchNormalization()(inp)
    for i in range(0, len(CHANNELS) - 1):
        channel = CHANNELS[i]
        x = layers.Conv2D(
            channel, kernel_size=(1, 1), activation="relu", padding=PADDING,
        )(x)
        x = layers.Conv2D(
            channel, kernel_size=KERNEL_SIZE, activation="relu", padding=PADDING,
        )(x)
        x = layers.Conv2D(channel, kernel_size=(
            1, 1), activation="relu", padding=PADDING,)(x)
        x = layers.AveragePooling2D(pool_size=POOL_SIZE, padding=PADDING, )(x)
        x = layers.Dropout(DROPOUT)(x)
        x = layers.BatchNormalization()(x)
    last_channel = CHANNELS[len(CHANNELS) - 1]
    x = layers.Conv2D(
        last_channel, kernel_size=(1, 1), activation="relu", padding=PADDING,
    )(x)
    x = layers.Conv2D(
        last_channel, kernel_size=KERNEL_SIZE, activation="relu", padding=PADDING,
    )(x)
    x = layers.Conv2D(last_channel, kernel_size=(
        1, 1), activation="relu", padding=PADDING)(x)
    x = layers.Flatten()(x)
    if DENSE_LAYER != 0:
    ...
    model = Model(inputs=inp, outputs=o, name="conv2d")
