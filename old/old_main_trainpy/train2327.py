...
def train_model(model_to_be_trained, spec_aug_params, audio_aug_params):
    ...
    for loader in audio_loaders:
        ...
        print("{} audios from {} have been loaded.".format(len(audios_dict[loader.name]), loader.name))

    # audio chunks
    audios_c_dict = {}
    for key, samples in audios_dict.items():
        preparer = return_preparer(key, samples) 
        preparer.prepare_all_samples()
        audios_c_dict[preparer.name] = preparer.return_all_samples()
        print("{} audios from {} have been prepared.".format(len(audios_c_dict[preparer.name]), preparer.name))

    # split
    train_audios_c_dict, val_audios_c_dict = split_and_extend(audios_c_dict, parameters.train_test_ratio)

    # spectrogram generation (val)
    val_samples = []
    for key, samples in val_audios_c_dict.items():
        generator = SpecGenerator(samples, key)
        generator.generate_all_samples()
        val_samples.extend(generator.return_all_samples())

    # b) 1) spectrogram generation (train)
    train_samples = []
    for key, samples in train_audios_c_dict.items():
        generator = SpecGenerator(samples, key)
        generator.generate_all_samples()
        train_samples.extend(generator.return_all_samples())
    original_training_length = len(train_samples)
    print("--- Original training dataset contains {} elements ---".format(len(train_samples)))

    # b) 2) spectrogram augmentation (train)
    spec_aug_train_samples = []
    ...
    # a) 2) spectrogram generation (not needed for augmix) (train)
    # aug_2_generator = SpecGenerator(audio_aug_samples, "audio_aug_train_samples")
    # aug_2_generator.generate_all_samples()
    # audio_aug_train_samples = generator.return_all_samples()

    # concatenating all the train elements
    train_samples.extend(spec_aug_train_samples)
...
if __name__ == "__main__":
    ...
    audio_aug_params = [["augmix", {"quantity" : 0.5, "label": -1, "no_pad" : False, "minval" : 0.3, "maxval" : 0.7, "aug_functions": [shift_pitch, stretch_time]}]]
    train_model(mixednet, spec_aug_params, audio_aug_params)
    initialize_job()
    audio_aug_params = [["augmix", {"quantity" : 0.5, "label": -1, "no_pad" : False, "minval" : 0.3, "maxval" : 0.7, "aug_functions": [stretch_time]}]]
    train_model(mixednet, spec_aug_params, audio_aug_params)
    initialize_job()
    audio_aug_params = [["augmix", {"quantity" : 0.8, "label": -1, "no_pad" : False, "minval" : 0.3, "maxval" : 0.7, "aug_functions": [shift_pitch, stretch_time]}]]
    train_model(mixednet, spec_aug_params, audio_aug_params)
    initialize_job()
    audio_aug_params = [["augmix", {"quantity" : 0.8, "label": -1, "no_pad" : False, "minval" : 0.3, "maxval" : 0.7, "aug_functions": [ stretch_time]}]]
    train_model(mixednet, spec_aug_params, audio_aug_params)
