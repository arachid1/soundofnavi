...
def train_model(datasets, model_to_be_trained, spec_aug_params, audio_aug_params, parse_function, validation_only):

    
    # simply initialize  audio loader object for each dataset
    # mandatory parameters:  root path; function for extracting filenames () 
    ...
    # ... but it's different for training because of augmentation. This functions sets up 2 branches:
...
def launch_job(datasets, model, spec_aug_params, audio_aug_params, parse_function, validation_only):
    initialize_job()
    train_model(datasets, model, spec_aug_params, audio_aug_params, parse_function, validation_only)
...
if __name__ == "__main__":
    ...
    launch_job({"Bd": 0, "Jordan": 1, "Icbhi": 1, "Perch": 0, "Ant": 0, "SimAnt": 0,}, mixednet, spec_aug_params, audio_aug_params, spec_parser, validation_only)
