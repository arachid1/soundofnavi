...
def train_model(datasets, model_to_be_trained, spec_aug_params, audio_aug_params, parse_function):
    ...
    # model setting
    model = model_to_be_trained(num_outputs=parameters.n_classes, frontend=frontend.Leaf(sample_rate=parameters.sr, n_filters=40), encoder=None)
    ...
    sps = []
    s = list(train_dataset.as_numpy_iterator())[0][0][:2]
