...
from modules.models.leaf_efnet1 import leaf_efnet1
...
class visualizationCallback_bis(tf.keras.callbacks.LambdaCallback):
    ...
    def on_epoch_begin(self, epoch, logs=None):
        ...
        for i, sample in enumerate(self.samples):
            ...
            output = self.model(audio, training=False)
            spec = np.squeeze(output.numpy())
...
class Encoder(tf.keras.Model):
    ...
    def __init__(self):
        ...
        self.inp = parameters.input_argument
        self.out = parameters.output_argument
        KERNEL_SIZE = 6
        POOL_SIZE = (2, 2)
        self.encoder = tf.keras.Sequential([
            InvertedResidual(filters=16, strides=1, kernel_size=KERNEL_SIZE, padding="same"),
            InvertedResidual(filters=16, strides=1, kernel_size=KERNEL_SIZE, padding="same"),
            layers.AveragePooling2D(pool_size=POOL_SIZE),
            layers.BatchNormalization(),
            layers.Dropout(0.1),
            InvertedResidual(filters=32, strides=1, kernel_size=KERNEL_SIZE, padding="same"),
            InvertedResidual(filters=32, strides=1, kernel_size=KERNEL_SIZE, padding="same"),
            layers.AveragePooling2D(pool_size=POOL_SIZE),
            layers.BatchNormalization(),
            layers.Dropout(0.1),
            InvertedResidual(filters=64, strides=1, kernel_size=KERNEL_SIZE, padding="same"),
            InvertedResidual(filters=64, strides=1, kernel_size=KERNEL_SIZE, padding="same"),
            layers.AveragePooling2D(pool_size=POOL_SIZE),
            layers.BatchNormalization(),
            layers.Dropout(0.1),
            # InvertedResidual(filters=128, strides=1, kernel_size=KERNEL_SIZE, padding="same"),
            # InvertedResidual(filters=128, strides=1, kernel_size=KERNEL_SIZE, padding="same"),
            # layers.AveragePooling2D(pool_size=POOL_SIZE),
            # layers.BatchNormalization(),
            # layers.Dropout(0.1),
        ...
        self.decoder = tf.keras.Sequential([
            ...
            # layers.BatchNormalization(),
            # layers.Dropout(0.1),
            InvertedResidual(filters=1, strides=1, kernel_size=KERNEL_SIZE, padding="same"),

            # InvertedResidual(filters=1, strides=1, kernel_size=KERNEL_SIZE,),
            # tf.keras.layers.Dense(1024, activation='sigmoid'),
            tf.keras.layers.Reshape(parameters.shape)
    ])
    ...
    def compute_loss(self, x, y, y_pred, sample_weight):
        ...
        # tf.print(tf.shape(x))
        mel_spec = self.out(x)
        # tf.print(tf.shape(mel_spec))
        mel_spec = tf.reshape(mel_spec, (-1,) + parameters.shape)
        # tf.print(tf.shape(mel_spec))
        mel_spec = mel_spec - tf.math.reduce_min(mel_spec)
        mel_spec = mel_spec/tf.math.reduce_max(mel_spec)

        # tf.print(tf.math.reduce_max(mel_spec))
        # tf.print(tf.math.reduce_min(mel_spec))
        # tf.print(tf.math.reduce_max(y_pred))
        # tf.print(tf.math.reduce_min(y_pred))
        ...
        loss = self.compiled_loss(mel_spec, y_pred, sample_weight, regularization_losses=self.losses)
        return loss

    def call(self, x):
        x = self.inp(x)
        x = tf.expand_dims(x, axis=-1)
        x = tf.reshape(x, (-1,) + parameters.shape)
        # print("here")
        # print(x.shape)
        encoded = self.encoder(x)
        # print(encoded.shape)
        decoded = self.decoder(encoded)
        decoded = decoded - tf.math.reduce_min(decoded)
        decoded = decoded/tf.math.reduce_max(decoded)
        # print(decoded.shape)
        return decoded
...
def train_model(datasets, model_to_be_trained, spec_aug_params, audio_aug_params, parse_function, label_passed):
    ...
    for i in range(len(train_audios_c_dict)):
        ...
        count = 0
        with open("official_labels.txt") as file:
            for line in file:
                trigger = True
        ...
        if parameters.oversample:
            none_samples = [s for s in _train_samples if s[1] == [0,0]]
            crackles_samples = [s for s in _train_samples if s[1] == [1,0]]
            wheezes_samples = [s for s in _train_samples if s[1] == [0,1]]
            both_samples = [s for s in _train_samples if s[1] == [1,1]]

            print("individual classes before oversampling")
            ...
            print("individual classes after oversampling")
            print(len(none_samples))
            print(len(crackles_samples))
            print(len(wheezes_samples))
            print(len(both_samples))
        ...
        viz_callback = visualizationCallback_bis(samples)
        ...
        # teacher.build(shape)
        # teacher._load(parameters.load_path, parameters.load_epoch)
        ...
        teacher.fit(train_dataset,
                ...
                callbacks=[tensorboard_callback, viz_callback]
        )
        ...
        # student = model_to_be_trained(num_outputs=parameters.n_classes, _frontend=parameters.frontend_argument)
        # shape = (None, parameters.audio_length*parameters.sr)
...
if __name__ == "__main__":
    ...
    parameters.sr = 16000
    parameters.icbhi_root = os.path.join(parameters.data_root, 'raw_audios/icbhi_preprocessed_v2_8000/')
    ...
    # parameters.load_path = "/home/alirachidi/classification_algorithm/cache/cw/train203/4/"
    # parameters.load_epoch = "19"
    parameters.n_fft = 2048
    parameters.window_len = 100
    parameters.window_stride = 25
    parameters.shape = (80, 200, 1)
    parameters.output_argument = leaf_frontend.MelFilterbanks(
    ...
    parameters.input_argument = leaf_frontend.Leaf(
            sample_rate=parameters.sr, 
            n_filters=parameters.n_filters, 
            # n_fft = parameters.n_fft,
            window_len = parameters.window_len,
    ...
    parameters._alpha = 0.1
    parameters.temperature = 1
    parameters.distill_features = False
    ...
    launch_job({"Bd": 0, "Jordan": 0, "Icbhi": 1, "Perch": 0, "Ant": 0, "SimAnt": 0,}, leaf_efnet1, spec_aug_params, audio_aug_params, None, [1,0])
    print("Job dir: {}".format(parameters.job_dir))
