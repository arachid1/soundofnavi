                ...
                with torch.no_grad():
                        for local_batch, local_labels in valloader:
                            ...
                            for index in indexes:
                                ...
                                sig = np.repeat([[0., 1., 0.], [0., 1., 0.]], 100)
                                print(f.shape)
                                print(win.shape)
                                print(sig.shape)
                                filtered = signal.convolve(sig,f, mode='same') / sum(f)
                                fig, ax_filt = plt.plot()
                                ax_filt.plot(filtered)
                                ax_filt.set_title('Filtered signal')
                                plt.savefig("{}/filter.png".format(folder))
                                exit()
