...
class leaf_CNN(tf.keras.Model):
  ...
  def __init__(self,
    ...
    self._pool = tf.keras.Sequential([
        layers.Conv2D(128, [7,11], strides = [2,2], padding = 'SAME'),
        ...
        layers.LeakyReLU(alpha = 0.1),
        layers.MaxPool2D(padding = 'SAME'),
        layers.Conv2D(512, [1,1], padding = 'SAME'),
        ...
        layers.Dense(num_outputs, activity_regularizer=tf.keras.regularizers.l2(parameters.ll2_reg), activation="sigmoid")
    ])
...
class MyLRSchedule(tf.keras.optimizers.schedules.LearningRateSchedule):
  ...
  def __call__(self, step):
    ...
    # self.ident
    return self.initial_learning_rate / tf.math.ceil((step+1)/ (2492 / parameters.batch_size))
...
def train_model(datasets, model_to_be_trained, spec_aug_params, audio_aug_params, parse_function, label_passed):
    ...
    # teacher
    teacher = model_to_be_trained(num_outputs=parameters.n_classes, _frontend=leaf_frontend.Leaf(sample_rate=16000, n_filters=80), normalize=True, encoder=None)
    ...
    optimizers = [
        tf.keras.optimizers.Adam(
        learning_rate=MyLRSchedule(5e-4, "frontend"), 
        beta_1=0.9, beta_2=0.999, epsilon=parameters.epsilon, decay=parameters.weight_decay, amsgrad=False
        ),
        tf.keras.optimizers.Adam(
        learning_rate=MyLRSchedule(5e-3, "backend"), 
        beta_1=0.9, beta_2=0.999, epsilon=parameters.epsilon, decay=parameters.weight_decay, amsgrad=False
        )
    ]
...
if __name__ == "__main__":
    ...
    spec_aug_params = []
    audio_aug_params = []
    launch_job({"Bd": 0, "Jordan": 0, "Icbhi": 1, "Perch": 0, "Ant": 0, "SimAnt": 0,}, leaf_CNN, spec_aug_params, audio_aug_params, None, [1,0])
