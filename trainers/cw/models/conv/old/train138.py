...
def train_model(datasets, model_to_be_trained, spec_aug_params, audio_aug_params, parse_function, label_passed):
    ...
    for i in range(len(train_audios_c_dict)):
        ...
        # teacher
        complex_conv_init = leaf_frontend.initializers.GaborInit(sample_rate=parameters.sr, max_freq=parameters.sr/2)
        teacher = model_to_be_trained(num_outputs=parameters.n_classes, _frontend=leaf_frontend.Leaf(
                                        sample_rate=parameters.sr, 
                                        preemp=parameters.preemp, 
                                        # window_len=64,
                                        # window_stride=32,
