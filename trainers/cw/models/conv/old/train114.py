...
def train_model(datasets, model_to_be_trained, spec_aug_params, audio_aug_params, parse_function, label_passed):
    ...
    for i in range(len(train_audios_c_dict)):
        ...
        # teacher
        teacher = model_to_be_trained(num_outputs=parameters.n_classes, _frontend=leaf_frontend.Leaf(sample_rate=16000, n_filters=80), normalize=True, encoder=None, weights=list(weights.values()), activate_spectral_loss=False)
