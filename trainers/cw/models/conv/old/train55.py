...
def train_model(datasets, model_to_be_trained, spec_aug_params, audio_aug_params, parse_function, label_passed):
    ...
    viz_callback = visualizationCallback(samples, save=False)

    gpus = tf.config.experimental.list_logical_devices('GPU')

    # teacher
    teacher = model_to_be_trained(num_outputs=parameters.n_classes, frontend=frontend.MelFilterbanks(sample_rate=16000, n_filters=80), encoder=None)
