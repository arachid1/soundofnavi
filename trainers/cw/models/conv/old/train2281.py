...
@tf.function
def mix_up_viz(ds_one, ds_two, batch_size, alpha=0.2):

    # Unpack two datasets
    images_one, labels_one = ds_one
    images_two, labels_two = ds_two
    # tf.io.write_file('mixup/images_one.txt', images_one)
    # tf.print("mix up")
    # tf.print(images_one)
    # tf.print(tf.reduce_sum(images_one))
    labels_one = tf.cast(labels_one, tf.float32)
    labels_two = tf.cast(labels_two, tf.float32)

    # Sample lambda and reshape it to do the mixup
    # l = sample_beta_distribution(batch_size, alpha, alpha)
    l = tf.random.uniform(shape=[], minval=0.4, maxval=0.6)
    tf.print(l)
    x_l = tf.reshape(l, (batch_size, 1, 1, 1))
    # x_l = l
    # tf.print(tf.shape(x_l))
    # print(tf.shape(x_l))
    ...
    return images_one, images_two, images_one_mod, images_two_mod, images, labels[0]
...
def train_model(train_file, job_dir, params, wav_params, spec_params, model_to_be_trained, job_count):
    ...
    if concat:
        ...
        # seed = 100 for one of the samples being only half
        train_dataset = train_dataset.shuffle(38)  # 20 with seed = 27 a rly weird example

        tf.random.set_seed(26)

        train_slice_one = train_dataset.filter(lambda x, y: y == 1)
        train_slice_one = train_slice_one.take(mixup_quantity) 
        
        train_slice_two = train_dataset.filter(lambda x, y: y == 0)
        train_slice_two = train_slice_two.take(mixup_quantity)

        # arr_slice_one = list(train_slice_one.as_numpy_iterator())
        # # print(arr_slice_one)
        # # print(np.sum(arr_slice_one[0][0]))
        # images_one = arr_slice_one[0][0]
        # images_one = np.squeeze(images_one)
        # visualize_spectrogram(images_one, 8000, os.path.join(folder, 'images_one.png'))

        # arr_slice_two = list(train_slice_two.as_numpy_iterator())
        # # print(arr_slice_two)
        # # print(np.sum(arr_slice_two[0][0]))
        # images_two = arr_slice_two[0][0]
        # images_two = np.squeeze(images_two)
        # visualize_spectrogram(images_two, 8000, os.path.join(folder, 'images_two.png'))

        # # print(np.sum(images_two))
        # # print(images_two.shape)
        # # first_half = images_two[0:128, 0:625]
        # # print(first_half.shape)
        # # print(np.sum(first_half))
        # # second_half = images_two[0:128,625:]
        # # print(second_half.shape)
        # # print(np.sum(second_half))

        train_slice = tf.data.Dataset.zip((train_slice_one, train_slice_two))

        mixup_batch_size = 1

        mixup_train_dataset = train_slice.map(lambda ds_one, ds_two: mix_up_viz(ds_one, ds_two, batch_size=mixup_batch_size, alpha=alpha))

        arr_full = list(mixup_train_dataset.as_numpy_iterator())
        print(len(arr_full))
        print(len(arr_full[0]))

        images_one = arr_full[0][0]
        images_one = np.squeeze(images_one)
        visualize_spectrogram(images_one, 8000, os.path.join(folder, 'images_one.png'))

        images_two = arr_full[0][1]
        images_two = np.squeeze(images_two)
        visualize_spectrogram(images_two, 8000, os.path.join(folder, 'images_two.png'))

        images_one_mod = arr_full[0][2]
        images_one_mod = np.squeeze(images_one_mod)
        visualize_spectrogram(images_one_mod, 8000, os.path.join(folder, 'images_one_mod.png'))

        images_two_mod = arr_full[0][3]
        images_two_mod = np.squeeze(images_two_mod)
        visualize_spectrogram(images_two_mod, 8000, os.path.join(folder, 'images_two_mod.png'))

        # # print(np.sum(images_two_mod))
        # # print(images_two_mod.shape)
        # # first_half = images_two_mod[0:128, 0:625]
        # # print(first_half.shape)
        # # print(np.sum(first_half))
        # # second_half = images_two_mod[0:128,625:]
        # # print(second_half.shape)
        # # print(np.sum(second_half))

        arr_full_mod = arr_full[0][4]
        arr_full_mod = np.squeeze(arr_full_mod)
        visualize_spectrogram(arr_full_mod, 8000, os.path.join(folder, 'mixup.png'))
        
        # print(arr_full[0])
        # first_half = arr_full[0][0:68, 0:625]
        # print(first_half.shape)
        # print(np.sum(first_half))
        # second_half = arr_full[0][0:68,625:]
        # print(second_half.shape)
        # print(np.sum(second_half))

        # print(arr_slice_two[0])
        # first_half = arr_full[0][0:68, 0:625]
        # print(first_half.shape)
        # print(np.sum(first_half))
        # second_half = arr_full[0][0:68,625:]
        ...
        # print(len(arr_slice_one))
        # print(len(arr_slice_two))
        # print(len(arr_full))
        # print(arr_slice_one[0][0][0])
        # print(arr_slice_two[0][0][0])
        # print(arr_full[0][0][0])
        exit()
...
if __name__ == "__main__":
    ...
    print("-----------------------")
    print("model 9")
    train_model(train_file, job_dir, params, wav_params, spec_params, model9, job_count=1)
    print("-----------------------")
