...
# from modules.models.leaf_model9_model_efnet0 import leaf_model9_model_efnet0
from modules.models.leaf_model9_model_efnet1 import leaf_model9_model_efnet1
# from modules.models.leaf_model9_model_efnet2 import leaf_model9_model_efnet2
# from modules.models.leaf_model9_model_resnet import leaf_model9_model_resnet
...
def train_model(datasets, model_to_be_trained, spec_aug_params, audio_aug_params, parse_function, label_passed):
    ...
    for i in range(len(train_audios_c_dict)):
        # if i == 0:
        #     print("already trained first kfold, next")
        #     continue
        initialize_job()
        ...
        teacher_2nd.set_weights(teacher.get_weights())
        ...
        teacher_3rd = model_to_be_trained(num_outputs=parameters.n_classes, _frontend=leaf_frontend.MelFilterbanks(
            sample_rate=parameters.sr, 
            n_filters=parameters.n_filters, 
            max_freq=float(parameters.sr/2)
        ))
        shape = (None, parameters.audio_length*parameters.sr)
        teacher_3rd.build(shape)
        teacher_3rd.set_weights(teacher.get_weights())
        
        teacher_3rd._pool.layers[0].trainable = True
        parameters.lr = 1e-6
        ...
        teacher_3rd.summary(line_length=110)

        history = teacher_3rd.fit(
            train_dataset,
            epochs=parameters.n_epochs,
            verbose=2,
            class_weight=weights,
