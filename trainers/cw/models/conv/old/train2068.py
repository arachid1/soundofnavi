...
@tf.function
def parse_pneumonia_function_multi(files, label, shape):
    ...
    for filename in files:
        ...
        spectrogram = tf.reshape(spectrogram.to_tensor(), shape)
        # spectrogram = tf.expand_dims(spectrogram, axis=0)
        spectrograms = spectrograms.write(i, spectrogram)
        i += 1
    # tf.print(tf.shape(spectrograms))/
    # tf.print(tf.shape(spectrograms))
    spectrograms = spectrograms.stack()
    # return spectrograms, label
    # return [[1], [2]], label
    return {"input_1": spectrograms[0], "input_2": spectrograms[1], "input_3": spectrograms[2], "input_4": spectrograms[3], "input_5": spectrograms[4], "input_6": spectrograms[5]}, label
...
def multi_model(N_CLASSES, SR, BATCH_SIZE, LR, SHAPE, WEIGHT_DECAY, LL2_REG, EPSILON, LABEL_SMOOTHING):

    model_a = conv2d(N_CLASSES, SR, BATCH_SIZE, LR, SHAPE, WEIGHT_DECAY, LL2_REG, EPSILON, LABEL_SMOOTHING)
    model_b = conv2d(N_CLASSES, SR, BATCH_SIZE, LR, SHAPE, WEIGHT_DECAY, LL2_REG, EPSILON, LABEL_SMOOTHING)
    model_c = conv2d(N_CLASSES, SR, BATCH_SIZE, LR, SHAPE, WEIGHT_DECAY, LL2_REG, EPSILON, LABEL_SMOOTHING)
    model_d = conv2d(N_CLASSES, SR, BATCH_SIZE, LR, SHAPE, WEIGHT_DECAY, LL2_REG, EPSILON, LABEL_SMOOTHING)
    model_e = conv2d(N_CLASSES, SR, BATCH_SIZE, LR, SHAPE, WEIGHT_DECAY, LL2_REG, EPSILON, LABEL_SMOOTHING)
    model_f = conv2d(N_CLASSES, SR, BATCH_SIZE, LR, SHAPE, WEIGHT_DECAY, LL2_REG, EPSILON, LABEL_SMOOTHING)

    combined = layers.concatenate([model_a.output, model_b.output, model_c.output, model_d.output, model_e.output, model_f.output])
    z = layers.Dense(32)(combined)
    z = layers.Dense(N_CLASSES, activity_regularizer=l2(LL2_REG), activation="sigmoid")(z)

    model = Model(inputs=[model_a.input, model_b.input, model_c.input, model_d.input, model_e.input, model_f.input], outputs=z)
...
def train_model(train_file, **args):
    ...
    with tf.device('/CPU:0'): 
        ...
        path = "/home/alirachidi/classification_algorithm/data/txt_datasets/all_sw_mel_preprocessed_v2_param_v30_augm_v0_cleaned_8000/"
        ...
        print("Number of 1-indexed chunks: {}".format(chunk_counts))
        ...
        train_dataset = train_dataset.map(lambda files, label: parse_pneumonia_function_multi(files, label, shape), num_parallel_calls=4)
        ...
        val_dataset = val_dataset.map(lambda files, label: parse_pneumonia_function_multi(files, label, shape), num_parallel_calls=4)
    ...
    # model setting
    model = multi_model(**model_params)
