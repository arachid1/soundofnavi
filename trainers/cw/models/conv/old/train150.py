...
def train_model(datasets, model_to_be_trained, spec_aug_params, audio_aug_params, parse_function, label_passed):
    ...
    for i in range(len(train_audios_c_dict)):
        ...
        _train_samples = _train_samples[:10]
        ...
        # w = h5py.File("/home/alirachidi/classification_algorithm/cache/cw/train140/2/_pool_29.h5")
        # print(w.keys())
        # print(w["efficientnetb1"])
        teacher._pool.load_weights("/home/alirachidi/classification_algorithm/cache/cw/train140/2/_pool_29.h5")
        
        teacher._pool.layers[0].trainable = False


        # print(teacher._pool.layers[1].weights - orig_weights)
        # exit()

        opt = tf.keras.optimizers.SGD(
            learning_rate=parameters.lr, momentum=0.3, nesterov=False, name="SGD"
        )
...
if __name__ == "__main__":
    ...
    parameters.sr = 16000
    parameters.one_hot_encoding = True
    ...
    # parameters.lr = 5e-3
    # parameters.n_epochs = 80
    # launch_job({"Bd": 0, "Jordan": 0, "Icbhi": 1, "Perch": 0, "Ant": 0, "SimAnt": 0,}, leaf_model9_model_res, spec_aug_params, audio_aug_params, None, [1,0])
    # print("Job dir: {}".format(parameters.job_dir))
