...
def train_model(datasets, model_to_be_trained, spec_aug_params, audio_aug_params, parse_function, sample_rate, n_filters):
    ...
    # model setting
    parameters.sr = sample_rate
    model = leaf_model(num_outputs=parameters.n_classes, frontend=frontend.Leaf(sample_rate=sample_rate, n_filters=n_filters), encoder=None)
...
def launch_job(datasets, model, spec_aug_params, audio_aug_params, parse_function, sample_rate, n_filters):
    '''
    parameters: 
    ...
    train_model(datasets, model, spec_aug_params, audio_aug_params, parse_function, sample_rate, n_filters)
...
if __name__ == "__main__":
    ...
    spec_aug_params = []
    audio_aug_params = []
    for sr in [12000, 20000]:
        for nf in [120]:
            launch_job({"Bd": 0, "Jordan": 0, "Icbhi": 1, "Perch": 1, "Ant": 1, "SimAnt": 1,}, leaf_model, spec_aug_params, audio_aug_params, None, sr, nf)
