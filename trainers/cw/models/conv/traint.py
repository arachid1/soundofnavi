...
def train_model(datasets, model_to_be_trained, spec_aug_params, audio_aug_params, parse_function):
    ...
    # tf datasets
    train_dataset, __, train_labels, __ = create_tf_dataset(train_samples, batch_size=parameters.batch_size, shuffle=True, parse_function)
    val_dataset, val_specs, val_labels, val_filenames = create_tf_dataset(val_samples, batch_size=1, shuffle=False, parse_function)
...
if __name__ == "__main__":
    ...
    spec_aug_params = []
    audio_aug_params = []
    launch_job({"Icbhi": 1, "Jordan": 1, "Bd": 0}, mixednet, spec_aug_params, audio_aug_params, generate_spec)
    
    # spec_aug_params = []
    # audio_aug_params = []
    # launch_job({"Icbhi": 0, "Jordan": 0, "Bd": 1}, mixednet, spec_aug_params, audio_aug_params, generate_spec)
