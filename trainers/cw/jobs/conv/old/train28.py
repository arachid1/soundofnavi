...
def train_model(datasets, model_to_be_trained, spec_aug_params, audio_aug_params, parse_function):
    ...
    # model setting
    model = model_to_be_trained(num_outputs=parameters.n_classes, frontend=frontend.SincNet(sample_rate=16000, n_filters=80), encoder=None)
    shape = (None, parameters.audio_length*parameters.sr)
    model.build(shape)

    optimizers = [
        tf.keras.optimizers.Adam(
            lr=1e-5, beta_1=0.9, beta_2=0.999, epsilon=parameters.epsilon, decay=parameters.weight_decay, amsgrad=False
        ),
        tf.keras.optimizers.Adam(
