...
def train_model(datasets, model_to_be_trained, spec_aug_params, audio_aug_params, parse_function, label_passed):
    ...
    for i in range(len(train_samples)):
        if train_samples[i][1] == [1, 0] or train_samples[i][1] == [0, 1] or train_samples[i][1] == [1, 1]: 
            train_samples[i][1] = 1
        else:
            train_samples[i][1] = 0
        
    for i in range(len(val_samples)):
        if val_samples[i][1] == [1, 0] or val_samples[i][1] == [0, 1] or val_samples[i][1] == [1, 1]: 
            val_samples[i][1] = 1
        else:
            val_samples[i][1] = 0
    ...
    # teacher
    teacher = model_to_be_trained(num_outputs=parameters.n_classes, frontend=frontend.Leaf(sample_rate=16000, n_filters=80), encoder=None)
    ...
    teacher._frontend.save_weights(parameters.job_dir + "/teacher_frontend_{}.h5".format(parameters.n_epochs))
    teacher._functional.save_weights(parameters.job_dir + "/teacher_functional_{}.h5".format(parameters.n_epochs))
