...
class custom_metric_callback(tf.keras.callbacks.LambdaCallback):
    ...
    def adaptive_lr_info(self):
        if self.model.optimizer.lr < 5e-2:
            self.model.optimizer.lr = self.model.optimizer.lr * 1.4
...
def generate_gru_spec(filename, label, shape, initial_channels, cuberooting):
...
def conv2d(N_CLASSES, SR, BATCH_SIZE, LR, SHAPE, WEIGHT_DECAY, LL2_REG, EPSILON, LABEL_SMOOTHING):
    ...
    model_10 = layers.Flatten()(model_10)
    o = layers.Dense(1, activation="softmax")(model_10)
    # delete above
...
def train_model(train_file, job_dir, params, wav_params, spec_params):
    ...
    if concat:
        ...
        val_dataset, train_dataset = process_data(val_samples, train_samples, generate_gru_spec, shape, batch_size, initial_channels, cuberooting)
    ...
    # callbacks
    metrics_callback = custom_metric_callback(val_dataset, shape, n_classes, job_dir, min_delta, es_patience, patience, min_lr, factor, epoch_start, target, split_count, clause)
