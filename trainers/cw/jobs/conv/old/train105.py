...
from modules.models.leaf_model9_model_103 import leaf_model9_model_103
...
class MyLRSchedule(tf.keras.optimizers.schedules.LearningRateSchedule):
  ...
  def __call__(self, step):
    ...
    # self.ident
    return self.initial_learning_rate / tf.math.ceil((step+1)/ (2.5*(2492 / parameters.batch_size)))
...
def train_model(datasets, model_to_be_trained, spec_aug_params, audio_aug_params, parse_function, label_passed):
    ...
    # teacher
    teacher = model_to_be_trained(num_outputs=parameters.n_classes, _frontend=leaf_frontend.Leaf(sample_rate=16000, n_filters=80), normalize=True, encoder=None, weights=list(weights.values()))
...
if __name__ == "__main__":
    ...
    spec_aug_params = []
    audio_aug_params = []
    launch_job({"Bd": 0, "Jordan": 0, "Icbhi": 1, "Perch": 0, "Ant": 0, "SimAnt": 0,}, leaf_model9_model_103, spec_aug_params, audio_aug_params, None, [1,0])
    print("Job dir: {}".format(parameters.job_dir))
