...
def train_model(train_file, job_dir, params, wav_params, spec_params):
    ...
    if concat:
        ...
        if spec_add:
            ...
            augmented_folder = os.path.join(dataset_path, "augmented_2")
            augmented_specs = apply_augmentations(augmented_specs, augmented_folder, len(augmented_specs), time_masking, frequency_masking, loudness, job_version, wav_path)
            print("The number of elements in the augmentation folder is: {}".format(len([name for name in os.listdir(augmented_folder) if os.path.isfile(os.path.join(augmented_folder, name))])))
...
if __name__ == "__main__":
    ...
    spec_params["TIME_MASKING"] = 0
    spec_params["FREQUENCY_MASKING"] = 0
    print("-----------------------")
    print("Job directory: {}".format(job_dir))
    print("Augmentation parameters for spectrograms: {}".format(spec_params))
    print("-----------------------")
