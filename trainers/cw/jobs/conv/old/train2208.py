...
def conv2d_1d(N_CLASSES, SR, BATCH_SIZE, LR, SHAPE, WEIGHT_DECAY, LL2_REG, EPSILON, LABEL_SMOOTHING):
    ...
    x = layers.TimeDistributed(layers.AveragePooling2D(pool_size=(1, POOL_SIZE), padding="same"))(x)
    ...
    x = layers.BatchNormalization()(x)
    # x = layers.Dropout(0.1)(x)
    x = TimeDistributedInvertedResidual(filters=64, strides=1, expansion_factor=3, kernel_size=3)(x)
    x = TimeDistributedInvertedResidual(filters=64, strides=1, expansion_factor=3, kernel_size=3)(x)
    x = layers.TimeDistributed(layers.AveragePooling2D(pool_size=(1, POOL_SIZE), padding="same"))(x)
    x = layers.BatchNormalization()(x)
    # x = layers.Dropout(0.1)(x)
    x = TimeDistributedInvertedResidual(filters=128, strides=1, expansion_factor=3, kernel_size=3)(x)
    # x = TimeDistributedInvertedResidual(filters=128, strides=1, expansion_factor=3, kernel_size=3)(x)
    x = layers.TimeDistributed(layers.AveragePooling2D(pool_size=(1, POOL_SIZE), padding="same"))(x)
    x = layers.BatchNormalization()(x)
    x = TimeDistributedInvertedResidual(filters=256, strides=1, expansion_factor=3, kernel_size=3)(x)
    # x = TimeDistributedInvertedResidual(filters=256, strides=1, expansion_factor=3, kernel_size=3)(x)
    x = layers.TimeDistributed(layers.AveragePooling2D(pool_size=(1, 8), padding="same"))(x)
    x = layers.BatchNormalization()(x)
    # x = layers.Dropout(0.1)(x)
