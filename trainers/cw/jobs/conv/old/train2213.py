...
def conv2d_1d(N_CLASSES, SR, BATCH_SIZE, LR, SHAPE, WEIGHT_DECAY, LL2_REG, EPSILON, LABEL_SMOOTHING):
    ...
    # x = TimeDistributedInvertedResidual(filters=48, strides=1, kernel_size=KERNEL_SIZE)(x)
    # x = TimeDistributedInvertedResidual(filters=48, strides=1, kernel_size=KERNEL_SIZE)(x)
    # x = layers.TimeDistributed(layers.AveragePooling2D(pool_size=POOL_SIZE, padding="same"))(x)
    # x = layers.BatchNormalization()(x)
    # x = layers.Dropout(0.1)(x)
    tower_1 = layers.TimeDistributed(layers.Conv2D(48, (1,1), padding='same', activation='relu'))(x)
    tower_1 = layers.TimeDistributed(layers.Conv2D(48, (3,3), padding='same', activation='relu'))(tower_1)
    tower_2 = layers.TimeDistributed(layers.Conv2D(48, (1,1), padding='same', activation='relu'))(x)
    tower_2 = layers.TimeDistributed(layers.Conv2D(48, (5,5), padding='same', activation='relu'))(tower_2)
    tower_3 = layers.TimeDistributed(layers.MaxPooling2D((3,3), strides=(1,1), padding='same'))(x)
    tower_3 = layers.TimeDistributed(layers.Conv2D(48, (1,1), padding='same', activation='relu'))(tower_3)
    x = layers.Concatenate(axis=-1)([tower_1, tower_2, tower_3])
    x = layers.TimeDistributed(layers.AveragePooling2D(pool_size=POOL_SIZE, padding="same"))(x)
    x = layers.BatchNormalization()(x)
    x = layers.Dropout(0.1)(x)
...
#     i = layers.Input(shape=SHAPE, )
#     x = layers.BatchNormalization()(i)
#     tower_1 = layers.TimeDistributed(layers.Conv2D(8, (1,1), padding='same', activation='relu'))(x)
#     tower_1 = layers.TimeDistributed(layers.Conv2D(8, (3,3), padding='same', activation='relu'))(tower_1)
#     tower_2 = layers.TimeDistributed(layers.Conv2D(8, (1,1), padding='same', activation='relu'))(x)
#     tower_2 = layers.TimeDistributed(layers.Conv2D(8, (5,5), padding='same', activation='relu'))(tower_2)
#     tower_3 = layers.TimeDistributed(layers.MaxPooling2D((3,3), strides=(1,1), padding='same'))(x)
#     tower_3 = layers.TimeDistributed(layers.Conv2D(8, (1,1), padding='same', activation='relu'))(tower_3)
#     x = layers.Concatenate(axis=-1)([tower_1, tower_2, tower_3])
#     x = layers.TimeDistributed(layers.AveragePooling2D(pool_size=POOL_SIZE, padding="same"))(x)
#     x = layers.BatchNormalization()(x)
#     # x = layers.Dropout(0.1)(x)
...
#     x = layers.BatchNormalization()(x)
#     # x = layers.Dropout(0.1)(x)
#     x = TimeDistributedInvertedResidual(filters=64, strides=1, expansion_factor=3, kernel_size=KERNEL_SIZE)(x)
#     x = TimeDistributedInvertedResidual(filters=64, strides=1, expansion_factor=3, kernel_size=KERNEL_SIZE)(x)
#     x = layers.TimeDistributed(layers.AveragePooling2D(pool_size=POOL_SIZE, padding="same"))(x)
#     x = layers.BatchNormalization()(x)
#     # x = layers.Dropout(0.1)(x)
#     x = TimeDistributedInvertedResidual(filters=128, strides=1, expansion_factor=3, kernel_size=KERNEL_SIZE)(x)
#     x = TimeDistributedInvertedResidual(filters=128, strides=1, expansion_factor=3, kernel_size=KERNEL_SIZE)(x)
#     x = layers.TimeDistributed(layers.AveragePooling2D(pool_size=POOL_SIZE, padding="same"))(x)
#     x = layers.BatchNormalization()(x)
#     x = TimeDistributedInvertedResidual(filters=256, strides=1, expansion_factor=3, kernel_size=KERNEL_SIZE)(x)
#     x = TimeDistributedInvertedResidual(filters=256, strides=1, expansion_factor=3, kernel_size=KERNEL_SIZE)(x)
#     x = layers.TimeDistributed(layers.AveragePooling2D(pool_size=POOL_SIZE, padding="same"))(x)
#     x = layers.BatchNormalization()(x)
#     # x = layers.Dropout(0.1)(x)
...
def train_model(train_file, job_dir, params, wav_params, spec_params):
    ...
    # if concat:
    #     val_dataset, train_dataset = process_data(grouped_val_samples, grouped_train_samples, concatenate_specs, shape, batch_size, initial_channels, cuberooting, 1)
    # else:
    #     print("-----------------------")
    ...
    #     val_dataset, train_dataset = process_data(val_samples, train_samples, generate_timed_spec, shape, batch_size, initial_channels, cuberooting, normalizing=1)
