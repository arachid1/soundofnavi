...
class MyLRSchedule(tf.keras.optimizers.schedules.LearningRateSchedule):
    ...
    def __call__(self, step):
        ...
        return self.initial_learning_rate / tf.math.pow(10.0, factor)
...
def train_model(datasets, model_to_be_trained, spec_aug_params, audio_aug_params, parse_function, label_passed):
    ...
    for i in range(len(train_audios_c_dict)):
        ...
        train_none_nb, train_crackles_nb, train_wheezes_nb, train_both_nb = train_labels.count([1,0,0,0]), train_labels.count([0,1,0,0]), train_labels.count([0,0,1,0]), train_labels.count([0,0,0,1])
        ...
        # teacher
        teacher = model_to_be_trained(num_outputs=parameters.n_classes, _frontend=leaf_frontend.MelFilterbanks(sample_rate=16000, n_filters=80), 
                            normalize=False, encoder=None, activate_weights=parameters.class_weights, activate_spectral_loss=parameters.activate_spectral_loss, activation="softmax")
...
if __name__ == "__main__":
    ...
    # launch_job({"Bd": 0, "Jordan": 0, "Icbhi": 1, "Perch": 0, "Ant": 0, "SimAnt": 0,}, leaf_model9_model_106_3, spec_aug_params, audio_aug_params, None, [1,0])
    # print("Job dir: {}".format(parameters.job_dir))
