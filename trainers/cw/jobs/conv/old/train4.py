                ...
                with torch.no_grad():
                        for local_batch, local_labels in valloader:
                            ...
                            for index in indexes:
                                ...
                                # fs = model.return_filters()
                                # f = fs[0].to("cpu").numpy()
                                # f = np.squeeze(f)

                                # Initializing value of x-axis and y-axis
                                # in the range -1 to 1
                                # x, y = np.meshgrid(np.linspace(-1,1,10), np.linspace(-1,1,3))
                                # dst = np.sqrt(x*x+y*y)
                                # # Initializing sigma and muu
                                # sigma = 1
                                # muu = 0.000
                                # # Calculating Gaussian array
                                # sig = np.exp(-( (dst-muu)**2 / ( 2.0 * sigma**2 ) ) )
                                # print("2D Gaussian array :\n")
                                # print(sig)
                                # print(sig.shape)

                                # mean = [1, 2]; std = 1; variance = np.square(std); cov = [[1,0], [0,1]]
                                # # x_axis = np.arange(-5,5,.01)
                                # # sig = np.exp(-np.square(x_axis-mean)/2*variance)/(np.sqrt(2*np.pi*variance))
                                # sig = np.random.multivariate_normal(mean, cov, (10000))
                                # print(sig.shape)
                                # sig = np.repeat([[1., 1., 1.]], 100)
                                # sig = np.reshape(sig, (100, f.shape[0]))
                                # win = signal.windows.hann(50)
                                # print(win.shape)

                                outputs, p = model(spec_to_track, return_spec=True)
                                no_train_out = np.squeeze(outputs.to("cpu").numpy())
                                visualize_spec_bis(p.cpu().numpy(), parameters.sr, "{}/p_{}_orig_".format(folder, index))
                                visualize_spec_bis(no_train_out, parameters.sr, "{}/outputs_{}_orig_".format(folder, index), title="index_{}_label_{}".format(index, label_to_track))
                                no_train_outs.append(no_train_out)
                                ps.append(p)

                                no_train_filt_outputs = model(spec_to_track, return_filt_spec=True)
                                tfo = np.squeeze(no_train_filt_outputs.to("cpu").numpy())
                                print(tfo.shape)
                                tfo = tfo[0,:,:]
                                print(tfo.shape)
                                visualize_spec_bis(tfo, parameters.sr, "{}/tfo_{}".format(folder, index))
                                exit()

                                
                                # o = np.squeeze(outputs.to("cpu").numpy())
                                # print(o.shape)
                                # print(f.shape)
                                # print(f)
                                # filtered = signal.convolve2d(o,f, "same") / sum(f)
                                # print(filtered.shape)
                                # fig, ax_filt = plt.figure()
                                # plt.figure()
                                # plt.plot(o)
                                # plt.savefig("{}/non_filtered.png".format(folder))
                                # plt.close()
                                # plt.figure()
                                # plt.plot(filtered)
                                # plt.savefig("{}/filtered.png".format(folder))
                                # plt.close()
                                exit()
                            break
                ...
                with torch.no_grad():
                        for local_batch, local_labels in valloader:
                            ...
                            for index in indexes:
                                ...
                                visualize_spec_bis(to_viz, parameters.sr, "{}/outputs_{}_diff".format(folder, index))
                                visualize_spec_bis(p_to_viz, parameters.sr, "{}/p_{}_diff".format(folder, index))
                                visualize_spec_bis(p.cpu().numpy(), parameters.sr, "{}/p_{}_end_".format(folder, index))
                                visualize_spec_bis(last_epoch_out, parameters.sr, "{}/outputs_{}_end_".format(folder, index), title="index_{}_label_{}".format(index, label_to_track))
                                i += 1
                            break
