...
def train_model(datasets, model_to_be_trained, spec_aug_params, audio_aug_params, parse_function, label_passed):
    ...
    for i in range(len(train_audios_c_dict)):
        ...
        teacher._load("/home/alirachidi/classification_algorithm/cache/cw/train162/5/", "15")
...
if __name__ == "__main__":
    ...
    parameters.frontend_argument = leaf_frontend.SincNet(
            sample_rate=parameters.sr, 
            n_filters=parameters.n_filters, 
            )

    filepath = "/home/alirachidi/classification_algorithm/trainers/cw/job_outputs/187.out"
    ...
    with open(filepath) as f:
        for line in f:
            if "Best icbhi_score:" in line:
                v = float(line.split(" ")[-1].strip())
                icbhi_scores.append(v)
            if "Best sensitivity:" in line:
                v = float(line.split(" ")[-1].strip())
                sensitivities.append(v)
            if "Best specificity:" in line:
                v = float(line.split(" ")[-1].strip())
                specificities.append(v)
            if "Best acc:" in line:
                v = float(line.split(" ")[-1].strip())
                accuracies.append(v)
    ...
    print("icbhi_scores")
    print(icbhi_scores)
    print(np.average(icbhi_scores))
    print(max(icbhi_scores))
    print("sensitivities")
    print(sensitivities)
    print(np.average(sensitivities))
    print(max(sensitivities))
    print("specificities")
    print(specificities)
    print(np.average(specificities))
    print(max(specificities))
    print("accuracies")
    print(accuracies)
    print(np.average(accuracies))
    print(max(accuracies))

    filepath = "/home/alirachidi/classification_algorithm/trainers/cw/job_outputs/162.out"
