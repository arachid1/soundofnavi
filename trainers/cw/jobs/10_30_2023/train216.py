...
def train_model(datasets, model_to_be_trained, spec_aug_params, audio_aug_params, parse_function, label_passed):
    ...
    train_audios_c_dict, val_audios_c_dict = split_and_extend(audios_c_dict, parameters.train_test_ratio, kfold=parameters.kfold)
    ...
    for i in range(len(train_audios_c_dict)):
        ...
        icbhi_val_dataset, _, _, icbhi_val_filenames = create_tf_dataset(icbhi_val_samples, batch_size=1, shuffle=False, parse_func=parse_function) # keep shuffle = False!
        icbhi_metrics_callback = NewCallback2(icbhi_val_dataset, icbhi_val_filenames, target_key="icbhi_score")
        ...
        history = teacher.fit(
            ...
            callbacks=[metrics_callback, icbhi_metrics_callback, viz_callback, tensorboard_callback]
        )
...
if __name__ == "__main__":
    ...
    launch_job({"Bd": 0, "Jordan": 0, "Icbhi": 1, "Perch": 1, "Ant": 1, "SimAnt": 0,}, leaf_pretrained, spec_aug_params, audio_aug_params, None, [1,0])
    print("Job dir: {}".format(parameters.job_dir))
