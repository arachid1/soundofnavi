...
class Model(pl.LightningModule):
    ...
    def on_after_backward(self):
        # freeze bins 
        self.spec_layer.wsin.grad[60:] = 0
        self.spec_layer.wcos.grad[60:] = 0
...
def train_model(datasets, model_to_be_trained, spec_aug_params, audio_aug_params, parse_function):
    ...
    with torch.no_grad():
            for local_batch, local_labels in testloader:
                ...
                visualize_spec_bis(no_train_out, parameters.sr, "temp/outputs", title="orig_index_{}_label_{}".format(index, label_to_track))
                break
    ...
    with torch.no_grad():
            for local_batch, local_labels in testloader:
                ...
                visualize_spec_bis(last_epoch_out, parameters.sr, "temp/outputs", title="orig_index_{}_label_{}".format(index, label_to_track))
                break

    to_viz = last_epoch_out - no_train_out 
    visualize_spec_bis(to_viz, parameters.sr, "temp/diff")
