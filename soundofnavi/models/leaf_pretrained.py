...
class leaf_pretrained(tf.keras.Model):
    ...
    def __init__(
        ...
        if parameters.model == "resnet":
            architecture = tf.keras.applications.resnet50.ResNet50(
                include_top=False,
                weights="imagenet",
                input_tensor=None,
            ...
            architecture = tf.keras.applications.EfficientNetB1(
                include_top=False,
                weights="imagenet",
                input_tensor=None,
                input_shape=parameters.spec_shape,
        ...
        self._pool = tf.keras.Sequential(
            [
                # tf.keras.layers.RepeatVector(n=parameters.n_initial_channels),
                architecture,
    ...
    #     spec = self._frontend(self.input_layer)
    #     # print("ehre")
    #     # print(spec.shape)
    #     self._pool(spec)
    ...
    def analyze_layers(self, domain_examples):
        self.focus_layers = [
            self.get_layer("leaf"),
            self.get_layer("sequential").get_layer("resnet50").get_layer("conv1_relu"),
            self.get_layer("sequential").get_layer("dense"),
        ]
    ...
    def compute_weights(self, y):
        ...
        if parameters.use_class_weights:
            ...
            sample_weight = tf.map_fn(
                fn=lambda t: tf.gather(parameters.weights, tf.cast(t, tf.int32)),
                dtype=tf.float64,
                elems=sample_weight,
            )
    ...
    def train_step(self, x, y):
        with tf.GradientTape() as tape:
            ...
            loss_value = self.compute_loss(x, y, y_pred, sample_weight=sample_weight)
