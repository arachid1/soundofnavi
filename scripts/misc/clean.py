...
def test_threshold(args):
    src_root = args.src_root
    wav_paths = glob("{}/**".format(src_root), recursive=True)
    wav_path = [x for x in wav_paths if x == str(args.fn + ".wav") in x]
    # print(len(wav_path))
    # print(wav_path)
    wav_path = ["data/dataset_labeled/none/9.wav"]
    if len(wav_path) == 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sr", type=int, default=44100, help="rate to downsample audio"
    )
    parser.add_argument(
        "--fn", type=str, default="9", help="file to plot over time to check magnitude"
    )
    parser.add_argument(
        "--threshold",
        type=str,
