...
def extract_wav(root, sr, overlap_threshold, audio_length, step_size, extension='.wav'):
    
    if "PERCH" in root or "perch" in root:
    ...
    for file_name in filenames:
        if file_name == '226_1b1_Pl_sc_LittC2SE':
            continue
        if "ICBHI" in root or "Antwerp" in root:
            data = get_multiple_samples(annotated_dict[file_name], file_name, root, sr, overlap_threshold, audio_length, step_size, extension) 
        else:
            data = get_single_sample(annotated_dict[file_name], file_name, root, sr, audio_length, extension)
        cycles_with_labels = []
        for d in data[1:]:
            cycles_with_labels.append((d[0], d[1], d[2], file_name))
