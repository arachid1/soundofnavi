...
def convert_to_coch(data, coch_path, coch_params):
    ...
    for d in data:
        if (not (len(d[0]) == 80000)):
            continue
        coch_a = np.loadtxt(coch_path + 'COCH_A4_80.txt', delimiter=',')
        coch_b = np.loadtxt(coch_path + 'COCH_B4_80.txt', delimiter=',')
        order = np.loadtxt(coch_path + 'p_order4_80.txt', delimiter=',')
